...
В текущей версии ML baseline обучается отдельной командой как корректор
физически интерпретируемой аналитической оценки:

- `RandomForestRegressor` прогнозирует остаточную ошибку
  `RUL_oracle_h - RUL_analytic_h`.
- Итоговый ML-прогноз рассчитывается как
...
- Входы: разрешенные наблюдаемые поля, инженерные признаки feature builder и
  аналитический RUL.
...
Статические PNG остаются в `outputs/<scenario_name>/plots/` как архивные артефакты
завершенного запуска:
...
- `12_doverie_k_dannym.png` - доверие к данным, средняя доля пропусков и согласованность прогнозов.
- `13_dolya_vybrosov.png` - часовая доля строк с инъецированными выбросами.
