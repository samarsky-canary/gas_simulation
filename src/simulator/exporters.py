...
OPERATION_DESCRIPTIONS = [
    ...
    },
    {
        ...
        "description": "Добавление измерительного шума, дрейфа давления и расчет наблюдаемого перепада по абсолютным давлениям.",
    },
    {
    ...
    {
        "step": "10. Экспорт",
        "description": "Сохранение наблюдаемых данных, скрытых меток, полного debug-набора и metadata.json.",
    },
]
    ...
    dataset = pd.DataFrame(
        {
            ...
            "Q_m3h": df["q_m3h"],
            "T_C": df["t_c"],
            "q_nominal_m3h": cfg.q_nominal_m3h,
            "p_in_nominal_mpa": cfg.p_in_nominal_mpa,
            "dp0_kpa": cfg.dp0_kpa,
            "k_s_per_hour": cfg.k_s_per_hour,
            "k_c": cfg.k_c,
            "beta": cfg.beta,
            "gamma_load": cfg.gamma_load,
...
def _operations_markdown() -> str:
    ...
    lines.append("- `metadata.json` - конфигурация, seed, QC-отчет и описание операций.")
    lines.append("")
    return "\n".join(lines)
...
def _dataset_schema_markdown() -> str:
    ...
    rows = [
        ...
        ("q_nominal_m3h", "float", "м3/ч", "Паспортный номинальный расход фильтра."),
        ("p_in_nominal_mpa", "float", "МПа", "Номинальное входное давление."),
        ("dp0_kpa", "float", "кПа", "Перепад давления на чистом фильтре."),
        ("k_s_per_hour", "float", "1/ч", "Базовая скорость накопления засорения."),
        ("k_c", "float", "-", "Коэффициент влияния засорения на сопротивление."),
        ("beta", "float", "-", "Нелинейность влияния засорения на сопротивление."),
        ("gamma_load", "float", "-", "Степень влияния расхода на скорость деградации."),
